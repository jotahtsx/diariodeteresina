<!DOCTYPE html>
<html lang="pt-br" x-data="{ adminDarkMode: localStorage.getItem('admin-theme') === 'dark' }" x-init="$watch('adminDarkMode', val => {
...
<body class="antialiased transition-colors duration-300 bg-white dark:bg-[#0b1120]">
    <div class="flex min-h-screen">
        ...
        {{-- Sidebar --}}
        <aside
            class="w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col sticky top-0 h-screen transition-colors z-[60]">
            <div class="p-8 pb-12">
                <a href="{{ route('admin.dashboard') }}" class="flex flex-col gap-0 group">
                    ...
                    <span class="text-[9px] font-black uppercase tracking-[0.4em] text-portal-blue mt-1">Portal de
                        Notícias</span>
                </a>
            </div>
            ...
            <nav class="flex-1 px-4 space-y-1.5 overflow-y-auto">
                ...
                <a href="{{ route('admin.dashboard') }}"
                    ...
                    <i
                        class="fa-solid fa-chart-pie w-5 {{ request()->routeIs('admin.dashboard') ? 'text-white' : 'text-slate-400 group-hover:text-portal-blue' }}"></i>
                    Dashboard
                </a>
                ...
                <a href="{{ route('admin.posts.index') }}"
                    class="flex items-center gap-3 px-4 py-3.5 rounded-lg {{ request()->routeIs('admin.posts.*') ? 'bg-slate-900 dark:bg-portal-blue text-white shadow-lg' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800' }} font-bold text-sm transition-all group">
                    <i class="fa-solid fa-tags w-5"></i>
                    Notícias
                </a>

                <p class="px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest mt-8 mb-4">Administração
                </p>
            ...
            <div class="p-4 mt-auto border-t border-slate-100 dark:border-slate-800">
                <a href="#"
                    class="flex items-center gap-3 px-4 py-3 rounded-lg text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 font-black uppercase text-[10px] tracking-widest transition-all">
        ...
        <main class="flex-1">
            <header
                class="h-20 bg-white dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-10 sticky top-0 z-50">
                <div class="flex items-center gap-4">
                    <div class="h-6 w-[3px] bg-portal-blue rounded-full"></div>
                    <span
                        class="text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Ambiente
                        de Gestão</span>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-4 group cursor-pointer">
                        ...
                        <div
                            class="w-10 h-10 rounded-lg bg-slate-100 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm transition-transform group-hover:rotate-3">
                            <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name ?? 'Admin') }}&background=1e40af&color=fff"
                                alt="User">
                        </div>
                    </div>
                </div>
