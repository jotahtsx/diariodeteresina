                ...
                @foreach ($postsPrincipais->skip(1) as $p)
                    <div class="group flex flex-col border-b border-slate-100 pb-6 mb-6">
                        ...
                        <a href="{{ route('site.post', $p->slug) }}" class="relative overflow-hidden mb-4 block"
                            ...
                            <img src="{{ Str::startsWith($p->image, ['http://', 'https://']) ? $p->image : ($p->image ? asset('storage/' . $p->image) : 'https://images.unsplash.com/photo-1585829365234-781f8c429478?q=80&w=1000') }}"
                                class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700">
                        </a>
                ...
                <div class="group flex flex-col border-b border-slate-100 pb-6">
                    <div class="mb-3 flex flex-col gap-1">
                        <div class="flex items-center gap-2">
                            <span
                                class="bg-slate-900 text-white px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider">{{ $c->city->name }}</span>
                            <span style="color: {{ $c->category->color ?? '#64748b' }};"
                                class="text-[11px] font-black uppercase tracking-tight">{{ $c->category->name }}</span>
                        </div>
                    </div>
                    <a href="{{ route('site.post', $c->slug) }}" class="relative overflow-hidden mb-4 block shadow-sm"
                        style="border-radius: 12px; height: 160px;">
                        <img src="{{ Str::startsWith($c->image, ['http://', 'https://']) ? $c->image : ($c->image ? asset('storage/' . $c->image) : 'https://images.unsplash.com/photo-1444653614773-995cb1ef9efa?q=80&w=1000') }}"
                    ...
                    <p class="mt-2 text-slate-500 text-xs line-clamp-2 leading-relaxed">
                        {{ Str::limit(strip_tags($c->content), 90) }}</p>
                </div>
            @endforeach
        </div>
    </div>
        ...
        <div class="flex flex-col items-center">
            ...
            <div style="max-width: 970px; min-height: 90px;"
                ...
                <div class="flex items-center gap-6 px-8 text-slate-500 group-hover:text-slate-300 transition-colors">
                    ...
                    <div class="flex flex-col">
                        <span class="text-[11px] font-bold tracking-widest uppercase">Anuncie no Diário de Teresina</span>
                        <span class="text-[9px] opacity-40 tracking-normal font-medium mt-0.5 uppercase">Formato Premium
                            970x90 — Comercial</span>
                    </div>
                </div>
            </div>
        </div>
